...
function on_button_press(action: AppAction) {
    ...
    if (sum_ducks_alive < 1) {
        game.setGameOverMessage(false, "Rounds Survived: " + rounds_survived)
...
function boot_up() {
    ...
    //  Select random action A
    let action_a = possible_actions_list[randint(0, possible_actions_list.length - 1)]
    ...
    //  Select random action B
    let action_b = possible_actions_list[randint(0, possible_actions_list.length - 1)]
    // random.choice(list(possible_actions.keys()))
    while (action_b == action_a) {
        action_b = possible_actions_list[randint(0, possible_actions_list.length - 1)]
    }
    // random.choice(list(possible_actions.keys()))
